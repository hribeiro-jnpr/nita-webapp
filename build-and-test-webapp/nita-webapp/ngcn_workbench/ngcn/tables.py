...
class AllActionHistoryTable(tables.Table):
    ...
    timestamp = tables.Column(verbose_name="Timestamp")
    status = tables.Column(verbose_name="Status")
    jenkins_job_build_no = tables.TemplateColumn(
        '<a href="'
        + jenkins_server_url
        + '/job/{{record.action_id.jenkins_url}}/{{record.jenkins_job_build_no}}/console" target="_blank" ">{{record.jenkins_job_build_no}}</a>'
    )
