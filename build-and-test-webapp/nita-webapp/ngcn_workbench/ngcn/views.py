...
def campusNetworkView(request, campus_network_id):
    return render(
        request, "ngcn/campus_network.html", {"campus_network_id": campus_network_id}
    )
        ...
        try:
            ...
            if form.is_valid():
                ...
                if result != "invalid_host":
                    ...
                    sheets = data_manager.get_sheets_by_campus_network(
                        campus_network_id
                    )
            ...
            return JsonResponse(
                {
                    "workbook": "",
                    "status": "failure",
                    "message": "Error while loading the Excel file",
                }
            )
    else:
            ...
            return JsonResponse(
                {
                    "workbook": "",
                    "status": "failure",
                    "message": "Error while parsing loading the data",
                }
            )
...
def configurationView(request, campus_network_id):
    return render(
        request,
        "ngcn/campus_network/upload_file.html",
...
def actionsView(request, campus_network_id, action_category_id):
    ...
    queryset = ActionHistory.objects.filter(
        campus_network_id=campus_network_id, category_id=category.id
    )
    ...
    if campus_networks:
        ...
        actions = Action.objects.filter(
            action_category=category, campus_type_id=campus_type_obj_id
        )
    actionsJson = []
    for action_obj in actions:
        actionJson = {
            "name": action_obj.action_name,
            "url": action_obj.jenkins_url,
            "id": action_obj.id,
        }
    ...
    return render(
        request,
        "ngcn/campus_network/noob.html",
        {"table": table, "actions": actionsJson},
    )
...
def actionHistoryDetailView(request, campus_network_id, action_category_id):
    queryset = ActionHistory.objects.filter(
        campus_network_id=campus_network_id,
        category_id=ActionCategory.objects.get(pk=action_category_id),
    ).order_by("-timestamp")
    table = ActionHistoryTable(queryset)
    RequestConfig(request, paginate=False).configure(table)
    return render(
        request, "ngcn/campus_network/campus_action_history.html", {"table": table}
    )
            ...
            return JsonResponse(
                {
                    ...
                    + _("network_type_heading")
                    + " has active "
                    + _("network_heading")
                    + "(s). Please delete the active "
                    + _("network_heading")
                    + "s before deleting the "
                    + _("network_type_heading"),
                    "name": name,
                }
            )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            src = (
                ...
                + "/"
                + campusNetwork.campus_type.app_zip_name
            )
            ...
            for job in jobs_list:
                ...
                logger.debug(
                    ...
                    + str(configuration["shell_command"])
                )
                logger.debug(
                    "configuration[output_path] " + str(configuration["output_path"])
                )
                logger.debug(
            ...
            if wait_and_get_build_status(action_url, current_build_number):
                logger.info(
                    "Campus Network - "
                    + campusNetwork.name
                    + " creation network_template_manager job finished successfully"
                )
                campusNetwork.save()
                ...
                logger.error(
                    "Campus Network - "
                    + campusNetwork.name
                    + " creation network_template_manager job failed."
                )
                ...
                server.build_job(
                    action_url,
                    {"operation": "delete", "src": src, "network_name": network_name},
                )
            ...
            return render(
                request,
                "ngcn/campus_network/add_campus_network.html",
                {
                    "form": form,
                },
            )
...
def triggerAction(request, action_id, campus_network_id):
    ...
    try:
        ...
        history = ActionHistory(
            action_id=Action.objects.get(pk=action_id),
            timestamp=timezone.now(),
            status="Running",
            jenkins_job_build_no=current_build_number,
            category_id=ActionCategory.objects.get(pk=action_obj.action_category.id),
            campus_network_id=CampusNetwork.objects.get(pk=campus_network_id),
        )
        history.save()
...
def createActionHistory(queryset, host):
    class AllActionHistoryTable(tables.Table):
        ...
        jenkins_job_build_no = tables.Column(
            verbose_name="Jenkins Job Id"
        )  # tables.TemplateColumn('<a href="http://'+host+'/job/{{record.action_id.jenkins_url}}/{{record.jenkins_job_build_no}}/console" target="_blank" ">{{record.jenkins_job_build_no}}</a>')
        console_log_link = tables.TemplateColumn(
            '<a href="https://'
            + os.getenv("JENKINS_URL", "localhost")
            + ":"
            + os.getenv("JENKINS_PORT", "8443")
            + '/job/{{record.action_id.jenkins_url}}-{{record.campus_network_id.name}}/{{record.jenkins_job_build_no}}/console" target="_blank" ">View</a>'
        )
