...
class CampusTypeForm(forms.Form):
    app_zip_file = forms.FileField(
        label="Application Zip File",
        widget=forms.FileInput(
            attrs={"class": "app_zip_file_upload_class", "id": "app_zip_file_upload"}
        ),
    )
...
class UploadFileForm(forms.Form):
    up_file = forms.FileField(
        ...
        widget=forms.FileInput(
            attrs={"class": "file_upload_class", "id": "file_upload"}
        ),
    )
...
ActionFormSet = inlineformset_factory(
    CampusType,
    Action,
    fields=("action_name", "jenkins_url", "action_category"),
    extra=0,
    min_num=1,
    validate_min=True,
