    ...
    def parseProjectFile(self, file_name):
        ...
        if zip_validation["status"]:
            try:
                ...
                tree.getElementsByTagName("customWorkspace")[
                    0
            ...
            result = JsonResponse(
                {"result": "failure", "reason": zip_validation["message"]}
            )
        ...
        for root, dirs, files in os.walk(tmp + "/tmp2/" + projectname):
            for file in files:
                zf.write(
                    ...
                    os.path.join(root, file).replace(tmp + "/tmp2/", ""),
                )
        zf.close()
        ...
        try:
            ...
            for member in list_of_files:
                if app_name + "/" + member not in archive_member_list:
                    ...
                    result["status"] = False
                    result["message"] = (
                        member + " is not present in the project directory"
                    )
                    logger.error(result)
                    return result
    ...
    def validateProjectYaml(self, project_file):
        ...
        try:
            ...
            if not campus_type_name or campus_type_name.isspace():
                error_string = (
                    "Invalid Campus Type name. Campus Type name must be empty"
                )
                return error_string
            ...
            if not project_file_dict["action"]:
                error_string = (
                    "Invalid project.yaml file. Actions must not be must be empty"
                )
                return error_string
            ...
            for action in project_file_dict["action"]:
                ...
                if (
                    ActionCategory.objects.filter(category_name=category_name).count()
                    == 0
                ):
                ...
                if not jenkins_url or jenkins_url.isspace():
                    error_string = (
                        "Invalid Action jenkins_url. The jenkins_url must not be empty."
                    )
                    return error_string
                ...
                if not shell_command or shell_command.isspace():
                    error_string = (
                        "Invalid shell command. The shell command must not be empty."
                    )
                    return error_string
            ...
            with transaction.atomic():
                ...
                for action in project_file_dict["action"]:
                    action_category = ActionCategory.objects.get(
                        category_name=action["category"].strip().upper()
                    )
                    ...
                    if (
                        "custom_workspace" in action["configuration"].keys()
                        and not action["configuration"]["custom_workspace"].isspace()
                    ):
                        custom_workspace_val = action["configuration"][
                            "custom_workspace"
                        ]
